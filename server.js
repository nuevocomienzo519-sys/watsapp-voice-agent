...
// ==========================================
// Endpoint: recibe el "code" del Embedded Signup (mandado automáticamente
// por el frontend de /conectar-whatsapp) y lo intercambia por un token de
// acceso, y de paso obtiene los negocios/WABAs asociados para poder
// identificar el número que se acaba de conectar en modo coexistencia.
// ==========================================
app.post("/conectar-whatsapp", async (req, res) => {
  try {
    ...
    // 3. Responder con lo obtenido (para inspección antes de automatizar el registro)
    res.json({
      ok: true,
      ...
      FB.login(
        function (response) {
          if (response.status === 'connected' && response.authResponse) {
            ...
            document.getElementById('resultado').textContent = 'Código recibido. Conectando con el servidor...';
            ...
            fetch('/conectar-whatsapp', {
              ...
              .catch(err => {
                document.getElementById('resultado').innerHTML =
                  '<p class="error">⚠️ Error al conectar con el servidor: ' + err.message + '</p>';
              });
          } else {
