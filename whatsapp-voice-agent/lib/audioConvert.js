...
/**
 * Convierte un buffer de audio (mp3, típicamente lo que devuelve ElevenLabs)
 * a ogg/opus mono 16kHz, que es el formato que WhatsApp necesita para
 * mostrar el audio como "nota de voz" (con forma de onda) en vez de un
 * archivo adjunto normal.
 *
 ...
 *
 * @param {Buffer} inputBuffer - audio de entrada (ej. mp3)
 * @returns {Promise<Buffer>} audio en formato ogg/opus
 */
async function convertToOggOpus(inputBuffer) {
  const tmpId = crypto.randomUUID();
  const inputPath = path.join(os.tmpdir(), `${tmpId}-in.mp3`);
