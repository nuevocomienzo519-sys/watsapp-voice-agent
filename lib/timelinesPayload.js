 ...
 * { messageId, chatId, direction, phone, attachment: { url, type, filename } | null, text }
 */
function parseTimelinesPayload(rawBody) {
  ...
  // Teléfono del cliente (para asociar la tarea de HubSpot al contacto
  // correcto). Se busca primero en el remitente del mensaje, y si no,
  // en el chat mismo (chats 1-a-1).
  ...
  if (rawAttachment) {
    const url = firstDefined(
      ...
      rawAttachment.link,
      rawAttachment.temporary_download_url
    );
    const type = firstDefined(
      rawAttachment.type,
      rawAttachment.mime_type,
      rawAttachment.content_type,
