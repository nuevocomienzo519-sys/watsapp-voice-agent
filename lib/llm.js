 ...
 * el formato de entrega (voz/texto/voz_y_texto) y qué acciones tomar en
 * HubSpot (mover etapa, asignar proyecto/asesor, crear tareas) según la
 ...
 * @param {{chatId: string, phone: string}} [contexto] - Datos del chat/contacto.
...
async function generateReply(userText, inputMode, contexto = {}) {
  ...
  // OJO: no asumir que el texto está en content[0] — Claude puede incluir
  // bloques de tipo "thinking" antes del bloque "text" real. Hay que
  // buscar específicamente el bloque de texto, no tomar el primero a ciegas.
  ...
  // Red de seguridad: si el cliente escribió por audio, NUNCA debe salir
  // solo texto (regla de negocio), sin importar lo que haya decidido el
  // modelo. Si Claude se equivocó, lo corregimos aquí en vez de tronar.
  if (inputMode === "audio" && formato === "texto") {
    console.warn("Claude devolvió FORMATO: texto para un mensaje de audio; forzando a voz.");
    formato = "voz";
  }

  // Red de seguridad: si por cualquier motivo el mensaje quedó vacío
  // (el modelo no incluyó el texto, algún separador salió mal, etc.),
  // no mandamos un mensaje vacío a TimelinesAI (lo rechaza con error).
  if (!texto) {
    console.warn(
  ...
  // Procesa las acciones en HubSpot según lo que Claude detectó.
...
async function procesarIntencion({ intencion, proyecto, detalle, contexto }) {
  ...
  let contact = phone ? await findContactByPhone(phone) : null;
  const esClienteNuevo = !contact;

  // Si el contacto no existe todavía en HubSpot, TimelinesAI/HubSpot
  // normalmente ya lo habrá creado por su cuenta al sincronizar el mensaje
  // (event message:received:new). Reintenta una vez por si acaba de
  // crearse justo antes de este procesamiento.
  if (!contact && phone) {
    contact = await findContactByPhone(phone);
  }

  const contactId = contact?.id;
  let deal = contactId ? await findDealByContactId(contactId) : null;
  ...
  // Si es primera conversación y ya tenemos deal, asigna proyecto y
  // asesor por turnos (esto solo debe pasar una vez, al arrancar).
  if (esClienteNuevo && dealId) {
    if (proyecto) await asignarProyecto(dealId, proyecto);
    await asignarAsesorRoundRobin(dealId);
    await moverEtapaDeal(dealId, "appointmentscheduled");
  }
}
