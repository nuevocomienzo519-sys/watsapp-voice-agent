...
// -----------------------------------------------------------------------
// Sugerencia de respuesta para el panel del equipo.
//
// A diferencia de generateReply (que le contesta directo al cliente y
// dispara efectos secundarios en HubSpot), esto es SOLO texto propuesto
// para que un asesor humano lo revise en el panel de conversaciones y
// decida si lo usa, lo edita, o lo ignora. Nunca se manda solo.
// -----------------------------------------------------------------------
function construirSystemPromptSugerencia() {
  const seccionProyectos = construirSeccionProyectos();
  const seccionFinanciamiento = construirSeccionFinanciamiento();
  return `
Eres el asistente de Nuevo Comienzo, una inmobiliaria en Franco, Silao.
Estás ayudando a un ASESOR HUMANO del equipo a redactar su próxima
respuesta a un cliente por WhatsApp — el asesor va a leer tu propuesta,
editarla si quiere, y mandarla él mismo; tú no le hablas directo al
cliente en esta tarea.

Información real de los proyectos (usa solo estos datos; si el dato que
te piden no aparece aquí, dilo con naturalidad en vez de inventarlo):

${seccionProyectos}
${seccionFinanciamiento ? `\n${seccionFinanciamiento}\n` : ""}
Instrucciones:
- Lee el historial de la conversación (mensajes anteriores) y redacta el
  siguiente mensaje que el asesor debería mandarle al cliente,
  continuando la conversación de forma natural.
- Responde en español, cálido y cercano, como asesor de ventas de
  confianza — no acartonado, no repitas frases hechas.
- Los montos de dinero van con número y símbolo de $ y comas (ej.
  "$1,700,000"), porque esto se manda como texto normal, no como voz.
- Máximo 4-5 oraciones.
- Devuelve ÚNICAMENTE el texto del mensaje sugerido, sin explicaciones,
  sin comillas, sin prefijos como "Sugerencia:" — solo el mensaje tal
  como se mandaría al cliente.
`;
}
// Junta mensajes consecutivos del mismo rol (la API de Claude exige
// alternancia estricta user/assistant) y asegura que la lista termine en
// "user", agregando un empujón neutral si el último mensaje ya fue
// nuestro (para que Claude tenga algo a qué responder).
function prepararHistorialParaSugerencia(historyCrudo) {
  const normalizado = [];
  for (const m of historyCrudo) {
    const anterior = normalizado[normalizado.length - 1];
    if (anterior && anterior.role === m.role) {
      anterior.content += "\n" + m.content;
    } else {
      normalizado.push({ role: m.role, content: m.content });
    }
  }
  while (normalizado.length && normalizado[0].role !== "user") {
    normalizado.shift();
  }
  if (!normalizado.length) {
    return [{ role: "user", content: "(sin mensajes previos todavía)" }];
  }
  if (normalizado[normalizado.length - 1].role !== "user") {
    normalizado.push({
      role: "user",
      content:
        "(No hay mensaje nuevo del cliente todavía; propón un mensaje de seguimiento natural basado en la conversación de arriba.)",
    });
  }
  return normalizado;
}
async function generarSugerencia(historyCrudo = []) {
  const systemPrompt = construirSystemPromptSugerencia();
  const messages = prepararHistorialParaSugerencia(historyCrudo);
  ...
  if (!res.ok) {
    const body = await res.text();
    throw new Error(`Error llamando a Claude (sugerencia): ${res.status} - ${body}`);
  }
