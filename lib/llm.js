...
- Si el cliente muestra interés en Diamante o Santuario (pregunta por
  ...
  todas las opciones disponibles, agrega el link del catálogo. NUNCA lo
  metas en medio de una oración: va SIEMPRE al final de tu respuesta,
  en su propia línea, después de un salto de línea en blanco, con este
  formato exacto:
 ...
 * detectada por Claude. El deal ya existe cuando esto se ejecuta (lo crea
 * un Workflow de HubSpot al llegar el chat exportado), así que solo lo
 * buscamos, nunca lo creamos.
 *
 * La asignación de proyecto/asesor para cliente nuevo corre SIEMPRE,
 * incluso si la intención es "ninguna" (ej. un simple "Hola") — por eso
 * va antes del filtro de intención, no después.
 */
async function procesarIntencion({ intencion, proyecto, detalle, contexto }) {
  ...
  if (esClienteNuevo && dealId) {
    ...
    console.log(`Cliente nuevo detectado, asesor asignado por turnos, deal ${dealId}`);
  }
