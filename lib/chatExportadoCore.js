...
const { subirArchivoTimelinesAI, enviarAdjuntoAChat } = require('./timelinesClient');
...
// Palabras/extensión que sugieren que un adjunto podría ser documentación
// importante (identificación, comprobante de domicilio, etc.) en vez de una
// foto normal del chat — solo para avisar, no bloquea nada.
const PALABRAS_CLAVE_DOCUMENTO = /ine|identificaci[oó]n|comprobante|domicilio|\.pdf$/i;
...
async function crearNegocio({ nombreCliente, proyecto, asesor }) {
  ...
  // Nota: esta etapa se fija aquí explícitamente y no hay ningún workflow
  // de HubSpot que la mueva automáticamente después — el negocio se queda
  // en "Base de datos" hasta que alguien la cambie a mano.
...
/**
 * Lista los nombres de los archivos adjuntos dentro del zip (fotos, PDFs,
 * etc.), excluyendo el propio chat.txt. Si el archivo es un .txt suelto
 * (sin zip), no hay adjuntos que listar.
 */
function listarArchivosAdjuntos(buffer, filename = '') {
  const pareceZip = /\.zip$/i.test(filename) || buffer.slice(0, 2).toString() === 'PK';
  if (!pareceZip) return [];
  const zip = new AdmZip(buffer);
  return zip
    .getEntries()
    .filter((e) => !/\.txt$/i.test(e.entryName))
    .map((e) => e.entryName);
}

/**
 * Arma el texto de resumen de adjuntos para la confirmación: cuántos
 * archivos hay y si alguno parece ser un documento importante (por
 * palabras clave en el nombre), a diferencia de una simple foto del chat.
 */
function resumenDocumentos(nombresArchivos) {
  if (nombresArchivos.length === 0) return 'No se encontraron archivos adjuntos.';
  const posiblesDocumentos = nombresArchivos.filter((n) => PALABRAS_CLAVE_DOCUMENTO.test(n));
  const totalFotos = nombresArchivos.length - posiblesDocumentos.length;
  let resumen = `${nombresArchivos.length} archivo(s) adjunto(s)`;
  if (totalFotos > 0) resumen += ` (${totalFotos} foto${totalFotos === 1 ? '' : 's'})`;
  if (posiblesDocumentos.length > 0) {
    resumen += `. ⚠️ Posible(s) documento(s) importante(s): ${posiblesDocumentos.join(
      ', '
    )} — revísalo manualmente.`;
  } else {
    resumen += '. No se detectaron documentos importantes.';
  }
  return resumen;
}
...
async function resumirConversacion(mensajes) {
  ...
  const conversacion = mensajes.map((t, i) => `${i + 1}. ${t}`).join('\n');
  try {
    const res = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 200,
        system:
          'Vas a recibir los mensajes de una conversación de ventas de bienes ' +
          'raíces por WhatsApp. Resume en máximo 2-3 líneas, en español, de qué ' +
          'trata: qué le interesa al cliente, qué preguntó, y en qué quedó la ' +
          'plática. Sé concreto, sin relleno. Si no hay suficiente información ' +
          'para resumir, responde solo "Sin información suficiente para resumir."',
    ...
    console.error('[chat-exportado] Error resumiendo conversación:', err.message);
    return null;
  }
}

async function actualizarTelefonoContacto(contactId, telefono) {
  return hubspotFetch(`/crm/v3/objects/contacts/${contactId}`, {
    method: 'PATCH',
    body: JSON.stringify({ properties: { phone: telefono } }),
  });
}

async function enviarTarjetaContacto(chatId, nombreCliente, telefono) {
  const vcard = construirVCard(nombreCliente, telefono);
  const archivoSubido = await subirArchivoTimelinesAI(
    Buffer.from(vcard, 'utf8'),
    `${nombreCliente.replace(/\s+/g, '_')}.vcf`,
    'text/vcard'
  );
  await enviarAdjuntoAChat(chatId, archivoSubido.uid);
}

/**
 * Arma el contenido de una tarjeta de contacto (.vcf) para el cliente
 * detectado, lista para subir y compartir en WhatsApp.
 */
function construirVCard(nombre, telefono) {
  const nombreLimpio = (nombre || 'Contacto').replace(/[\r\n]/g, ' ').trim();
  return ['BEGIN:VCARD', 'VERSION:3.0', `FN:${nombreLimpio}`, `TEL;TYPE=CELL:${telefono}`, 'END:VCARD'].join(
    '\r\n'
  );
}
 ...
 * @param {string} filename nombre real del archivo tal como se mandó (para mostrarlo en la confirmación)
 * @param {string} textoMensaje texto completo del mensaje de WhatsApp (caption), para
 *   extraer el teléfono si el usuario lo incluyó (ej. "4721234567")
 * @param {string|number|null} chatId id del chat de TimelinesAI donde se mandó el archivo —
 *   se usa para compartir ahí mismo la tarjeta de contacto del cliente (respuesta dentro
 *   de la conversación existente, no un chat nuevo)
 */
async function procesarChatExportado(buffer, filename = '', textoMensaje = '', chatId = null) {
  ...
  const nombresAdjuntos = listarArchivosAdjuntos(buffer, filename);
  const resumenAdjuntos = resumenDocumentos(nombresAdjuntos);
  ...
  // En vez de iniciar un chat NUEVO con el cliente (lo cual consume el
  // crédito limitado de "nuevo mensaje" de TimelinesAI), compartimos la
  // tarjeta de contacto del cliente directo en la MISMA conversación donde
  // llegó el zip — es una respuesta dentro de un chat existente, así que
  // cualquier persona del equipo puede tocarla y escribirle al cliente
  // cuando quiera, sin depender de crédito agotado.
  const resumenConversacion = await resumirConversacion(datos.mensajesCliente);
