...
// Parsea el _chat.txt que viene dentro del ZIP de "Exportar chat" de WhatsApp (formato iPhone).
...
/**
 * Extrae los mensajes crudos del texto del _chat.txt
 * @param {string} rawText contenido de _chat.txt (utf8)
...
function extraerMensajes(rawText) {
  ...
  const msgRegex = /\[(\d{2}\/\d{2}\/\d{2}), (\d{1,2}:\d{2}:\d{2}\s?[ap]\.\s?m\.)\]\s([^:]+):\s?([\s\S]*?)(?=\n?\[\d{2}\/\d{2}\/\d{2},|$)/g;
...
/**
 * Punto de entrada principal: recibe el texto del _chat.txt y regresa
...
function parseChatExport(rawText) {
  ...
  if (mensajes.length === 0) {
    throw new Error('No se encontraron mensajes reconocibles en el _chat.txt');
  }
